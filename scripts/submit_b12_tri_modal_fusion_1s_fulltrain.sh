...
export_csv+=",ENCODER_LORA_ACTIVATION_CHECKPOINTING=1"
...
echo "[submit-tri-modal-fulltrain] tag=${TAG}"

if command -v sbatch >/dev/null 2>&1; then
  sbatch --time="${SLURM_TIME}" --export="${export_csv}" "${RUN_SCRIPT}"
else
  echo "[submit-tri-modal-fulltrain] sbatch not found; running locally"
