...
RoPE (no new params): in every AttentivePooler self-attn block
  Q'_p = R(p) Q_p,  K'_p = R(p) K_p,  θ_{p,m} = p · ω_m
with p = original Frame/slot Index surviving prune (abs_stream). Cross-attn
rotates K only (learnable queries have no frame id).

Fair accuracy needs probe finetune (RoPE changes Q/K geometry). Arms:
  baseline       original ckpt, no probe RoPE
  rope_zeroshot  original ckpt + temporal RoPE (shows why FT is needed)
  rope_ft        probe/heads FT with temporal RoPE on, then eval with RoPE
"""
...
def encode_pruned_128(stream: StreamKVAttnPruneEncoder, clips: torch.Tensor, embed_dim: int):
...
def eval_arm(stream, mtp_clf, embed_dim, loader, device, ck_meta, rope: ProbeTemporalRoPE | None, prefix: str):
    ...
    for batch in loader:
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            tok, frame_ids = encode_pruned_128(stream, clips, embed_dim)
            if rope is not None:
...
def plot_results(payload: dict, png: Path, copy_png: Path | None):
    ...
    ax.set_title(f"128 KV + 34 attn-prune → probe   val={n}")
...
def main():
    ...
    ap.add_argument("--out-dir", type=Path, default=Path(
        "/scratch/ll5914/experiments/probe_temporal_rope_abs_frame_128kv"
    ))
    ap.add_argument("--copy-json", type=Path, default=Path(
        "/home/ll5914/Jepa_yifan/probe_temporal_rope_abs_frame_128kv.json"
    ))
    ap.add_argument("--copy-png", type=Path, default=Path(
        "/home/ll5914/Jepa_yifan/probe_temporal_rope_abs_frame_128kv.png"
    ))
    ...
    # --- zero-shot RoPE ---
    rope = ProbeTemporalRoPE(pooler, rope_cross_attn_k=True)
    ...
    ft_table["n@2s"] = 0
    train_seconds = 0.0
    ft_ckpt = args.out_dir / "probe_temporal_rope_ft.pt"

    if not args.skip_finetune:
        logger.info("cache frozen encoder tokens + abs frame ids for probe FT")
        cached = []
        with torch.no_grad():
        ...
        mtp_clf.train()
        opt = torch.optim.AdamW([p for p in mtp_clf.parameters() if p.requires_grad], lr=args.lr, weight_decay=1e-4)
        ...
        torch.save(
            {
                ...
                "epochs": args.epochs,
                "lr": args.lr,
                "rope": "temporal_1d_abs_frame_index",
    ...
    payload = {
        "method": METHOD,
        "note": (
            "Probe 1D temporal RoPE with original Frame/slot Index after 128+34 "
            "attn prune. Finetune probe+heads; encoder frozen."
        ),
        ...
        "config": {
            ...
            "rope": "1D temporal on probe self-attn Q/K (+ cross-attn K); abs stream slot id",
            "formula": "Q'=R(p)Q, K'=R(p)K, theta_p,m = p * omega_m, p=original Frame Index",
            "epochs": args.epochs,
            "lr": args.lr,
            "train_seconds": train_seconds,
            "ckpt": str(args.nopred_ckpt),
            "ckpt_meta": {k: ck_meta[k] for k in ("epoch", "step", "phase")},
            "ft_ckpt": str(ft_ckpt) if ft_ckpt.exists() else None,
        },
    }
    png = args.out_dir / "probe_temporal_rope_abs_frame_128kv.png"
