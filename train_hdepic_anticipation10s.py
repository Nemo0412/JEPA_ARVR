...
def run(from_scratch=False):
    ...
    print(f"  Train annotations: {len(train_df)} rows / val: {len(val_df)} rows", flush=True)
    ...
    print(f"  Valid train samples: {len(train_ds)} / val: {len(val_ds)} (after {anticipation_sec}s filter)", flush=True)
    if len(train_ds) == 0:
