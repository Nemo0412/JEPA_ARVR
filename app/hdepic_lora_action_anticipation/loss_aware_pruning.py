"""Loss-aware token pruning for V-JEPA-style video encoders (project-local port).

Ported from ``refer_repo/JEPA_ARVR/loss_aware_pruning.py`` (ll's method). The
reference tree is read-only and is never imported here; this is a self-contained
copy adapted for the B13 EGTEA streaming-MTP frozen KV-cache ablation.

Token importance (calibration only) uses a first-order Taylor approximation of
...
Runtime pruning applies a **saved calibrated policy**:
  ...
  * token ranking via calibrated mean importance scores (position-indexed)
...
class LossAwarePruningConfig:
    ...
    # Project extension: which loss the calibration gradients come from.
    calibration_objective: str = "anticipation"  # anticipation | jepa
...
def save_calibration_artifacts(
    ...
    torch.save(
        {
            "per_layer_mean_token_scores": {
                int(k): v.detach().cpu().float() for k, v in per_layer_mean_token_scores.items()
            },
...
def compute_token_importance(z: torch.Tensor, grad: torch.Tensor) -> torch.Tensor:
    """Gradient-based score: mean_D(|grad * z|), shape [B, N]."""
    if grad is None:
        raise ValueError("compute_token_importance requires grad from loss backward")
    ...
    if keep_count is None:
        if conditional_prune_ratio is not None:
            keep_count = conditional_keep_count(
                num_tokens, conditional_prune_ratio, gp=gp, round_to_frame_tokens=round_to_frame_tokens
            )
        elif keep_ratio is not None:
            keep_count = conditional_keep_count(
                num_tokens, keep_ratio, gp=gp, round_to_frame_tokens=round_to_frame_tokens
            )
        else:
...
def verify_cascade_global_ratio(
    ...
    *,
    ...
    """Assert product_l (1-r_l) ~= 1 - global_prune_ratio; return achieved keep ratio."""
    ...
    *,
    ...
    for layer in sorted(per_layer_prune_ratios):
        keep = conditional_keep_count(
            n, per_layer_prune_ratios[layer], gp=gp, round_to_frame_tokens=round_to_frame_tokens
        )
        out[layer] = keep
        n = keep
    return out
...
def expected_final_keep_count(
    ...
    *,
    ...
    counts = simulate_cascade_keep_counts(
        num_tokens_full, per_layer_prune_ratios, gp=gp, round_to_frame_tokens=round_to_frame_tokens
    )
    if not counts:
...
def lookup_calibrated_scores(
    ...
    # Guard against streaming contexts shorter than the calibrated full length:
    # clamp positions to the table range (position-indexed prefix semantics).
    pos = token_pos.clamp_max(table.numel() - 1)
    return table[pos]
...
def build_prune_ratio_schedule(
    ...
    return allocate_cascade_prune_ratios(layer_sensitivity_map, config.global_prune_ratio, eps=config.eps)


class LossAwareScoreCollector:
    """Capture intermediate encoder features and gradients at pruning layers."""
...
def accumulate_position_scores(
    ...
    for layer, score in batch_scores.items():
        ...
        running_sensitivities[layer] += (
            float(batch_sensitivities[layer]) - running_sensitivities[layer]
        ) / count
    return count
...
def finalize_calibration_config(
    ...
    *,
    ...
    out._calibrated_scores_cache = {  # type: ignore[attr-defined]
        int(k): v.detach().float().clone() for k, v in per_layer_mean_token_scores.items()
    }
    return out
