"""ViT encoder forward with loss-aware cascade token pruning (project-local port).

Ported from ``refer_repo/JEPA_ARVR/models/vit_encoder_pruning.py`` (ll's method).
The reference tree is read-only and never imported; this copy imports the
project-local :mod:`app.hdepic_lora_action_anticipation.loss_aware_pruning`.

``LossAwareEncoderPruner`` patches ``encoder.forward`` so the encoder itself
prunes tokens at intermediate layers using a saved calibrated policy (native
cascade), returning the already-pruned token sequence. Downstream the streaming
MTP wrapper (``PrunedAnticipativeModel`` with ``pruner=None``) runs the predictor
on that pruned sequence with its existing rebased-position convention.
"""
    ...
    for layer_idx, blk in enumerate(encoder.blocks):
        if encoder.use_activation_checkpointing:
            x = torch.utils.checkpoint.checkpoint(
                blk, x, token_pos, None,
                T=t, H_patches=h_patches, W_patches=w_patches, use_reentrant=False,
            )
        else:
...
class LossAwareEncoderPruner:
    """Runtime encoder patch applying a saved calibrated cascade pruning policy."""
        ...
        *,
        ...
        if not config.calibration_scores_path and not hasattr(config, "_calibrated_scores_cache"):
            raise ValueError(
                ...
                "(run offline calibration with loss gradients first)"
            )
    ...
    def _forward(self, x, masks=None):
        ...
        if self.config.protected_token_indices:
            protected = torch.tensor(
                self.config.protected_token_indices, device=x.device, dtype=torch.long
            )
